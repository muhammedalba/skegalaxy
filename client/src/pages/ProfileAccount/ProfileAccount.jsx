   ...
   if (updateError) {
    ...
    if (updateSuccess  ) {
      if (updateSuccess && password) {
          ...
          navigate("/login");
        } else{ successNotify("تمت التعديل بنجاح")}

    }
    
  //  no cookies
  }, [data, isSuccess, updateSuccess, navigate, updateError, updatedUser, password, error]);
        ...
        <form onSubmit={handleSubmit} className="m-auto p-3 mt-5">
          <Fade delay={0} direction="up" triggerOnce={true} cascade>
            ...
            <div className="d-flex align-items-center flex-wrap justify-content-between">
              <Link to={`/orders`} className=" fs-3 m-1">
                {" "}
                <MdOutlineWifiProtectedSetup color="green" /> الذهاب الى طلباتي{" "}
              </Link>
            </div>
