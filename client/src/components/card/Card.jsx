...
const PiEyeThin = lazy(() =>
  import("react-icons/pi").then((module) => ({ default: module.PiEyeThin }))
);
const IoShareSocial = lazy(() =>
  import("react-icons/io5").then((module) => ({
    default: module.IoShareSocial,
  }))
);
const SlSocialInstagram = lazy(() =>
  import("react-icons/sl").then((module) => ({
    default: module.SlSocialInstagram,
  }))
);
...
import { FacebookIcon, XIcon, WhatsappIcon, TelegramIcon } from "react-share";
...
const Card = ({ product, imgePath }) => {
  ...
  const [
    ...
    {
      error: createError,
      isSuccess: successCreeate,
      isLoading: createLoding,
      data: crerateData,
    },
  ...
  return (
    <div className="card-product mt-3 m-auto ">
      <Suspense>
        <IoShareSocial
          style={{ top: "4rem" }}
          className="cart-icon mx-2 mouse-hover top-4"
          onClick={showIcons}
        />
      </Suspense>
      <Suspense>
        <CiHeart
          onClick={() => addproducToCartOurWishlist(product?._id, "wishlist")}
          style={{ top: "1rem", left: "auto" }}
          className="cart-icon-Heart mx-2 start-0 mouse-hover"
        />
      </Suspense>
      <Link onClick={scrollToTop} to={`/products/${product?._id}`}>
        <Suspense>
          <PiEyeThin
            style={{ top: "1rem" }}
            className="cart-icon mx-2 mouse-hover"
          />
        </Suspense>

        <div
          style={{ transform: display ? "translateX(0)" : " translateX(158%)" }}
